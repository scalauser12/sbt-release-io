...
- **Typed context threading**: `ReleaseContext` carries VCS, execution flags, startup-only plan data, versions, and typed metadata through the step chain with type safety. The only intentional mirror onto `sbt.State` is the chosen version pair (`ReleaseKeys.versions`), so sbt tasks can compute tag names and commit messages from the live release version. sbt-release uses untyped `State` attributes (`state.get(key)`) as its primary coordination mechanism.
