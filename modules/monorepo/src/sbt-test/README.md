...
- Version files and tags: `custom-version-format`, `per-project-releaseversionfile`, `version-file-change-detection`, `shared-version-file`, `custom-tag-name`, `tag-exists-error`, `gitignored-version-file`, `duplicate-rendered-tags-abort-preflight`, `post-hook-duplicate-tags-abort-before-tag`, `post-hook-invalid-tag-aborts-before-tag`
- Tag preflight and remote tag probe: `invalid-tag-name-fails-preflight`, `hook-disabled-tag-preflight-still-probes`, `retry-tag-name-probes-remote`, `overwrite-answer-probes-remote`, `remote-only-tag-aborts-preflight`, `keep-tag-remote-divergence-aborts`
...
- Publish, push, and validation flow: `manifest-metadata`, `missing-publishto`, `missing-version-file`, `snapshot-dependencies`, `publish-artifacts-checks-disabled`, `publish-empty-validation-fails-closed`, `publish-frozen-false-no-extra-probe`, `publish-overlay-key-restored-runs-hooks`, `publish-post-skip-source-restored-runs-after-hook`, `publish-runs-after-tag`, `publish-skip-bypass`, `publish-skip-drift-frozen`, `publish-skip-scala-drift-fails-closed`, `publish-task-scala-drift-runs-after-hook`, `publish-skip-isSnapshot`, `publish-skip-isSnapshot-default-flow`, `publish-skip-eval-error`, `publish-to-eval-error`, `push-behind-remote`, `push-changes-tracking-remote`, `push-decision-no-skips-remote-warmup`, `selection-aware-validation`, `skip-publish`, `skip-tests`, `skip-tests-setting`, `vcs-signoff`
