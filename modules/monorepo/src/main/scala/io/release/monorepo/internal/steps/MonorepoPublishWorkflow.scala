...
import io.release.monorepo.MonorepoContext.PublishTargetProgress
...
/** Publish validation, hook-gating, and execution workflow. */
private[monorepo] object MonorepoPublishWorkflow {
  ...
  private[monorepo] def afterPublishGateValidation(
    ...
    publishGateValidation(ctx, project)
    ...
    MonorepoVersionWorkflow.withReleaseVersionOverlay(ctx) { tempState =>
      for {
        ...
        probe                = PublishValidationProbe(
                                 ...
                                 targetProgress =
                                   if (targetRequired)
                                     PublishTargetProgress.Pending(afterSkip)
  ...
  /** Install release-manifest metadata fallbacks for a project before its publish task runs.
    ...
    * [[MonorepoTagWorkflow.tagReleasesPerProject]] (when the tag is created). This helper
            ...
            ctx.vcs match {
              ...
              case Some(vcs) =>
                vcs.currentHash.flatMap { hash =>
                  IO.blocking {
                    val newState = MonorepoVersionFiles.appendSessionSettingsPreservingVersioning(
                      ctx.state,
  ...
  private[monorepo] def executePublish(
      ctx: MonorepoContext,
      project: ProjectReleaseInfo
  ): IO[MonorepoContext] =
    IO.blocking(publishIterationForState(ctx.state, project)).flatMap { iteration =>
  ...
  private[monorepo] def validatePublish(
      ctx: MonorepoContext,
      project: ProjectReleaseInfo
  ): IO[MonorepoContext] =
    preparePublishValidation(ctx.resetFinalizedPublishValidation)
      .flatMap { preparedCtx =>
        if (effectiveSkip(preparedCtx) || !preparedCtx.hasValidatedPublishEligibilitySnapshot)
          IO.pure(preparedCtx)
        else
          resolvePublishValidationProbe(preparedCtx, project).flatMap { case (probedCtx, probe) =>
            probe.targetProgress match {
              case PublishTargetProgress.Pending(afterSkipState)                       =>
                validatePublishTargetForProject(
                  probedCtx,
                  project,
                  probe,
                  afterSkipState
  ...
  /** Establish the run-level publish validation boundary before per-project
    ...
    * checks-disabled and direct execute paths.
    */
  private[monorepo] def preparePublishValidation(
    ...
    if (frozenCtx.publishValidationInitialized) IO.pure(frozenCtx)
    else
      publishChecksEnabled(frozenCtx).map(frozenCtx.initializePublishValidation)
  }
  ...
  private def validatePublishTargetForProject(
      ...
      project: ProjectReleaseInfo,
      probe: PublishValidationProbe,
      afterSkipState: State
  ): IO[MonorepoContext] =
    for {
      publishTarget <- evaluatePublishTargetAt(afterSkipState, project).map { case (_, target) =>
                         target
                       }
