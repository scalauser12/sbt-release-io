...
/** Version-related helpers shared across monorepo release step objects.
  *
  * Separated from [[MonorepoStepHelpers]] to keep version workflow logic
  * (prompting, consistency validation, file resolution, committing) in one place.
  */
private[monorepo] object MonorepoVersionHelpers {
  ...
  /** Validate that all projects agree on a version dimension. Raises on mismatch.
    *
    * This is a reusable utility called at multiple points by design:
    *  - `MonorepoVersionSteps.validateVersions` — standalone release step, catches
    *    mismatches early in global-version mode before any writes.
    *  - `commitVersions` below — precondition before committing, ensures consistency
    *    at write-time even if the validate step was skipped or steps were reordered.
    *  - `MonorepoVcsSteps.tagReleasesUnified` — precondition before creating a unified
    *    tag, since a single tag requires all projects to agree on the release version.
    */
