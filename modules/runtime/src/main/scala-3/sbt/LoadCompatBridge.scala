...
object ReleaseIOLoadCompatBridge:
  ...
  private final case class StructurePartition(
      persistentPrefixLength: Int,
      transient: Seq[Setting[?]]
  )

  private final case class PersistentDefinitionShape(
      key: ScopedKey[?],
      dependencies: Vector[ScopedKey[?]],
      definitive: Boolean,
  ...
  private def hasTrustedSessionStructureGuard(settings: Seq[Setting[?]]): Boolean =
    settings.exists(setting =>
      setting.asInstanceOf[AnyRef] eq trustedSessionStructureGuardSetting.asInstanceOf[AnyRef]
    )

  private def trustedSessionStructureGuardIsLive(structure: BuildStructure): Boolean =
    structure.data
      .getDirect(trustedSessionStructureGuardSetting.key)
      .exists(_ eq trustedSessionStructureGuardToken)

  private def guardSettingIfMissing(settings: Seq[Setting[?]]): Seq[Setting[?]] =
    if hasTrustedSessionStructureGuard(settings) then Nil
  ...
  private def withPersistentDefinitionProvenance[A](setting: Setting[A]): Setting[A] =
    if setting.key == trustedSessionStructureGuardSetting.key ||
  ...
  private def structurePartition(
      state: State,
      extracted: Extracted
  )(using Show[ScopedKey[?]]): StructurePartition =
    import extracted.*
    ...
    trustedMarker.filter(marker =>
      ...
      case None                                                            =>
        ...
        if !hasExpectedPersistentPrefix(current, expectedPrefix) ||
          !guardIsValid
  ...
  def promoteTransientSettingsByKey(
    ...
    if exactTrustedStructure.exists(_.persistentPrefixLength == current.length) then state
    else
      ...
      val definitionsByKey = current.foldLeft(
        Map.empty[ScopedKey[?], Vector[Setting[?]]]
      ) { (acc, setting) =>
        val scopedKey         = setting.key
        val activeDefinitions =
          if setting.definitive then Vector.empty[Setting[?]] :+ setting
          else acc.getOrElse(scopedKey, Vector.empty[Setting[?]]) :+ setting
      ...
      def effectiveDependency(key: ScopedKey[?]): Option[ScopedKey[?]] =
        val scopes = (key.scope +: structure.delegates(key.scope)).distinct
        scopes.iterator
          .map(scope => ScopedKey(scope, key.key))
          .find(definedKeys.contains)

      def delegatedBase(key: ScopedKey[?]): Option[ScopedKey[?]] =
        structure
          .delegates(key.scope)
          .iterator
      ...
      def activeDependencies(key: ScopedKey[?]): List[ScopedKey[?]] =
      ...
      def dependencyClosure(
          pending: List[ScopedKey[?]],
          visited: Set[ScopedKey[?]],
          required: Set[ScopedKey[?]]
      ): Set[ScopedKey[?]] =
        pending match
          case Nil                         => required
      ...
      if promoted.isEmpty then state
      else
