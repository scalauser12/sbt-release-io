...
- Publish and push flow: `publish-to-check`, `publish-skip`, `publish-skip-root`, `publish-skip-drift-frozen`, `publish-skipped-custom-action-filtered`, `publish-skip-isSnapshot`, `publish-skip-isSnapshot-default-flow`, `publish-runs-after-tag`, `skip-publish-setting`, `publish-multi-project`, `publish-multi-project-manifest`, `publish-multi-project-version-mismatch`, `publish-nested-aggregate`, `push-changes`, `push-explicit-tag`, `push-first-tracked-branch`, `push-behind-remote`, `push-race-before-push`, `custom-publish-action`
...
- Version-file and dependency edge cases: `missing-version-file`, `late-bound-missing-version-file`, `dash-prefixed-version-file`, `gitignored-version-file`, `untracked-version-file`, `version-file-outside-vcs`, `late-bound-version-file-outside-vcs`, `snapshot-deps`, `snapshot-deps-test-scope`, `snapshot-deps-cross`, `empty-commit`, `empty-commit-noop`
